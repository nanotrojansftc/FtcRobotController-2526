...
@Autonomous(name = "4th-Far Red Modular Auto Plus", group = "4thMeet")
public class FarRedModularAutonomousPlus extends LinearOpMode {
...
// --- INNER CLASS FOR PATHS ---
    public static class Paths {
        ...
        public Paths(Follower follower) {
            ...
            Path1 = follower.pathBuilder()
                    .addPath(new BezierLine(new Pose(87.500, 8.000), new Pose(83.9, 18.9)))
                    .setLinearHeadingInterpolation(Math.toRadians(90), Math.toRadians(65.9272))
                    .build();
            // PATH 2: Basket -> Setup Position for 1st Sample
            Path2 = follower.pathBuilder()
                    .addPath(new BezierLine(new Pose(83.9, 18.9), new Pose(110, 43)))
                    .setLinearHeadingInterpolation(Math.toRadians(65.9272), Math.toRadians(1))
                    .build();

            // PATH 3: Setup -> Pickup 1st Sample (INTAKE ON, 0.5x speed)
            Path3 = follower.pathBuilder()
                    .addPath(new BezierLine(new Pose(110.000, 43.000), new Pose(140.000, 43.000)))
    ...
    public void runOpMode() throws InterruptedException {
        ...
        follower.setStartingPose(new Pose(87.5, 8.0, Math.toRadians(90)));
...
//        runStep(robotPaths.Path7, "SHOOT", SubsystemTrigger.AFTER_PATH);
    ...
    private void runStep(PathChain path, String action, SubsystemTrigger trigger) {
        ...
        // If this is the Path3 intake step, scale chassis power to 0.5
        if ("INTAKE".equals(action) && path == robotPaths.Path3) {
            setDrivePowerScale(0.5);
        } else {
            setDrivePowerScale(1.0);
        }
        ...
        // Restore full power after the step
        setDrivePowerScale(1.0);
    }
    ...
    private void setDrivePowerScale(double scale) {
        scale = Math.max(0, Math.min(1, scale));  // clamp
        Constants.driveConstants.maxPower(scale);
    }
